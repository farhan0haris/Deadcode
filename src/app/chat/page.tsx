...
export default function ChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "assistant",
      content: "Hello! I am **DeadCode AI Assistant v3.0**. Ask me anything about your connected codebase, architecture, functions, or commit history.",
      timestamp: "Just now"
    }
  ]);
  ...
  const [threads, setThreads] = useState([
    { id: "t1", title: "Authentication & Prisma Setup", active: true },
    { id: "t2", title: "FastAPI Engine RAG Flow", active: false },
    { id: "t3", title: "On This Day Commit Parser", active: false }
  ]);
    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        {
          id: (Date.now() + 1).toString(),
          role: "assistant",
          content: "Sorry, I encountered an issue connecting to the AI processing engine. Please check your connection or switch privacy modes.",
          timestamp: "Just now"
        }
      ]);
    } finally {
    ...
    <div className="flex h-screen bg-[#071330] text-[#EBEBEB] font-sans overflow-hidden">
      ...
      <aside className="w-80 bg-[#0A1A3F]/90 border-r border-[#74B4D9]/20 flex flex-col p-4 space-y-4 hidden md:flex">
        ...
        <div className="space-y-1">
          <h2 className="text-xs font-semibold text-[#EBEBEB]/50 uppercase tracking-wider">Chat Threads</h2>
          <div className="space-y-1 pt-2">
            {threads.map(t => (
              <div 
                key={t.id}
                className={`flex items-center justify-between p-2.5 rounded-xl text-xs font-medium cursor-pointer transition-all ${
                  t.active ? "bg-[#10367D] text-white border border-[#74B4D9]/40" : "text-[#EBEBEB]/70 hover:bg-white/5"
                }`}
              >
                <div className="flex items-center gap-2 truncate">
                  <MessageSquare className="w-3.5 h-3.5 text-[#74B4D9]" />
        ...
        <div className="mt-auto p-3 bg-[#10367D]/30 border border-[#74B4D9]/20 rounded-xl space-y-2 text-xs text-[#EBEBEB]/70">
          <div className="flex items-center gap-2 font-bold text-[#74B4D9]">
            <BookOpen className="w-4 h-4" /> RAG Context Active
          </div>
          <p className="text-[11px] leading-relaxed">
            AI has full vector access to repository files and commit timeline logs.
          </p>
        </div>
      </aside>
        ...
        <header className="flex items-center justify-between p-4 lg:px-8 border-b border-[#74B4D9]/20 bg-[#10367D]/20 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#EBEBEB]/60">Repository Scope: Deadcode Workspace</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-6">
          ...
          {loading && (
            <div className="flex gap-4 max-w-4xl items-center text-xs text-[#74B4D9]">
              ...
              <p className="animate-pulse font-medium">Synthesizing codebase AST vector memory...</p>
            </div>
          )}
        </div>
        ...
        <div className="p-4 lg:p-6 border-t border-[#74B4D9]/20 bg-[#0A1A3F]/60 backdrop-blur-md">
          <form onSubmit={handleSend} className="max-w-4xl mx-auto flex items-center gap-3">
            <input
              ...
              placeholder="Ask anything about functions, components, or commits (e.g. 'How does authentication work?')..."
